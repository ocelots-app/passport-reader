import {DataHash, MrzInfo} from '../types';
...
export function hash(bytesArray: number[]) {
  ...
  const hashFunc = shajs('sha256');
  hashFunc.update(Uint8Array.from(unsignedBytesArray));
  return hexToSignedBytes(hashFunc.digest('hex'));
}
